...
namespace System.Collections.Generic
{
    ...
    public static class CollectionExtension
    {
        ...
        /// <example>
        ///   <code>list.AddUnique(1); // returns true;
        ///     list.AddUnique(1); // returns false the second time;</code>
        /// </example>
        public static bool AddUnique<T>(this ICollection<T> collection, T value)
        {
            var hasValue = collection.Contains(value);
            if (!hasValue) collection.Add(value);
            return hasValue;
        }
        ...
        {
            var uniqueStore = new Dictionary<int, T>();
        ...
        {
            if (null == collection) throw new ArgumentNullException("collection");
            if (default(Action<T>) == action) throw new ArgumentNullException("action");
